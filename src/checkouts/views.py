...
def checkout_redirect_view(request):
    ...
    print("stripe_id:",customer_stripe_id)
...
def checkout_finalize_view(request):
    ...
    try:
        _user_sub_obj = UserSubscription.objects.get(
            user=user_obj
        )
    ...
    if None in [sub_obj, user_obj, _user_sub_obj]:
        return HttpResponseBadRequest("There was an error with your subscription. Please contact support.")
    ...
    if _user_sub_exists:
        ...
        if old_stripe_id is not None and not same_stripe_id:
            try:
                ...
                helpers.billing.cancel_subscription(old_stripe_id, reason="Auto ended,new membertship", feedback="switched_service")
            except:
                pass
        ...
        _user_sub_obj.save()
        messages.success(request, "Checkout Success!. Thank you for your subscription.")
